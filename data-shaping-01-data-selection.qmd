...
y <- degree[, .(mcid, degree)]
y <- unique(y)

z <- degree[, .(mcid, term_degree)]
z <- unique(z)
...
degree[mcid == "MCID3111263269"]
degree[mcid == "MCID3111314169"]



x <- degree[, .SD[which.min(term_degree)], by = "mcid"]
x[duplicated(mcid)]

data(course)
x <- course[, .SD[which.min(term_course)], by = "mcid"]



x <- course[, .(mcid, term_course, abbrev, number)]
x <- x[, .SD[term_course == min(term_course)], by = "mcid"]
nrow(x)
x[anyDuplicated(x, by = "mcid")]


x[mcid == "MCID3111142225"]
```
